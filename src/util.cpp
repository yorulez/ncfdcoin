...
#else
    const char* pszModule = "caucasianmineralwaters";
#endif
    if (pex)
        return strprintf(
...
#else
    // Unix
    return pathRet / ".caucasianmineralwaters";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "caucasianmineralwaters.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No caucasianmineralwaters.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override caucasianmineralwaters.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "caucasianmineralwaters.pid"));
